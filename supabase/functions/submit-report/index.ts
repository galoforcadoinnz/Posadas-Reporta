...
function parseNamedKeys(value: string | undefined): string[] {
  if (!value) return []
  try {
    const record = JSON.parse(value) as Record<string, unknown>
    return Object.values(record).filter((item): item is string => typeof item === 'string')
  } catch {
    return []
  }
}
...
function secretKey(): string {
  const namedKeys = parseNamedKeys(Deno.env.get('SUPABASE_SECRET_KEYS'))
...
function defaultDependencies(): HandlerDependencies {
  ...
  return {
    ...
    rateLimitPepper: requireEnvironment('RATE_LIMIT_PEPPER'),
    publishableKeys,
...
export function createSubmitReportHandler(dependencies: HandlerDependencies) {
  return async (request: Request): Promise<Response> => {
    ...
    if (!apiKey || !dependencies.publishableKeys.has(apiKey)) {
      return jsonResponse({ error: 'Solicitud no autorizada.' }, 401, origin)
    }

    const ip = dependencies.trustedIp(request)
    if (!ip) {
      return jsonResponse({ error: 'No se pudo validar el origen de la solicitud.' }, 503, origin)
    }
    ...
    try {
      ...
      if (error instanceof RequestValidationError) {
        return jsonResponse({ error: 'Los datos del reporte no son válidos.' }, 400, origin)
      }
      return jsonResponse({ error: 'No se pudo procesar el reporte.' }, 500, origin)
    }

    const challengeIsValid = await dependencies.verifyChallenge({
    ...
    if (!challengeIsValid) {
      return jsonResponse({ error: 'La verificación antiabuso no fue válida.' }, 403, origin)
    }
    ...
    try {
      ...
      if (code === 'IDEMPOTENCY_CONFLICT') {
        return jsonResponse({ error: 'La solicitud no pudo reutilizarse.' }, 409, origin)
      }
      if (code === 'LOCATION_OUTSIDE_CITY') {
        return jsonResponse({ error: 'La ubicación está fuera del área habilitada.' }, 400, origin)
      }
      if (code === 'CITY_REPORTING_BOUNDS_UNAVAILABLE') {
        return jsonResponse(
          { error: 'Los reportes no están habilitados temporalmente.' },
          503,
          origin,
        )
      }
      return jsonResponse({ error: 'No se pudo guardar el reporte.' }, 500, origin)
    }
  }
}
